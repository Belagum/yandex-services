...
class LicenseWindow(tk.Toplevel):
    def __init__(self, parent: tk.Tk, on_ok):  # noqa: E501
        ...
        self.msg = tk.Label(self, fg="red")
        self.msg.pack(pady=(10, 0))

        tk.Label(self, text="Ключ:").pack(anchor="w", padx=10)
        self.var_key = tk.StringVar(value=self.store.load().get("license_key", ""))
        tk.Entry(self, textvariable=self.var_key).pack(fill="x", padx=10)

        tk.Button(self, text="Сохранить", command=self._save).pack(pady=10)
        self._validate()
